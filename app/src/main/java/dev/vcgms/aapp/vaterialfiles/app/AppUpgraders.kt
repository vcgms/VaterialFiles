...
import dev.vcgms.aapp.vaterialfiles.compat.PreferenceManagerCompat
...
import dev.vcgms.aapp.vaterialfiles.file.asExternalStorageUriOrNull
...
import dev.vcgms.aapp.vaterialfiles.navigation.StandardDirectorySettings
import dev.vcgms.aapp.vaterialfiles.provider.archive.ArchiveFileSystem
import dev.vcgms.aapp.vaterialfiles.provider.common.ByteString
import dev.vcgms.aapp.vaterialfiles.provider.common.moveToByteString
import dev.vcgms.aapp.vaterialfiles.provider.content.ContentFileSystem
import dev.vcgms.aapp.vaterialfiles.provider.document.DocumentFileSystem
import dev.vcgms.aapp.vaterialfiles.provider.document.resolver.ExternalStorageProviderHacks
import dev.vcgms.aapp.vaterialfiles.provider.linux.LinuxFileSystem
import dev.vcgms.aapp.vaterialfiles.provider.root.RootStrategy
import dev.vcgms.aapp.vaterialfiles.provider.sftp.SftpFileSystem
import dev.vcgms.aapp.vaterialfiles.provider.smb.SmbFileSystem
import dev.vcgms.aapp.vaterialfiles.storage.DocumentTree
import dev.vcgms.aapp.vaterialfiles.storage.FileSystemRoot
import dev.vcgms.aapp.vaterialfiles.storage.PrimaryStorageVolume
import dev.vcgms.aapp.vaterialfiles.util.StableUriParceler
import dev.vcgms.aapp.vaterialfiles.util.asBase64
import dev.vcgms.aapp.vaterialfiles.util.readParcelable
import dev.vcgms.aapp.vaterialfiles.util.readParcelableListCompat
import dev.vcgms.aapp.vaterialfiles.util.toBase64
import dev.vcgms.aapp.vaterialfiles.util.toByteArray
import dev.vcgms.aapp.vaterialfiles.util.use
        ...
        Parcel.obtain().use { newParcel ->
            ...
            Parcel.obtain().use { oldParcel ->
                ...
                repeat(size) {
                    ...
                    oldParcel.readInt()
                    val className = oldParcel.readString()
                    if (className == "dev.vcgms.aapp.vaterialfiles.storage.SmbServer") {
                        newParcel.writeInt(PARCEL_VAL_PARCELABLE)
                        newParcel.writeString("dev.vcgms.aapp.vaterialfiles.storage.SmbServer")
                        ...
                        newParcel.writeString(
                            "dev.vcgms.aapp.vaterialfiles.provider.smb.client.Authority"
                        )
                        ...
                        oldParcel.readString()
                        newParcel.writeString(
                            "dev.vcgms.aapp.vaterialfiles.provider.smb.client.Authentication"
                        )
...
private fun migratePath1_4_0(oldParcel: Parcel, newParcel: Parcel) {
    ...
    when (className) {
        ...
        "dev.vcgms.aapp.vaterialfiles.provider.sftp.SftpPath" ->
            newParcel.writeParcelable(oldParcel.readParcelable<SftpFileSystem>(), 0)
        "dev.vcgms.aapp.vaterialfiles.provider.smb.SmbPath" ->
            ...
            Parcel.obtain().use { oldParcel ->
                ...
                repeat(size) {
                    ...
                    when (oldParcel.readString()) {
                        "dev.vcgms.aapp.vaterialfiles.storage.SftpServer" -> {
                            newParcel.writeInt(PARCEL_VAL_PARCELABLE)
                            newParcel.writeString("dev.vcgms.aapp.vaterialfiles.storage.SftpServer")
                            val id = oldParcel.readLong()
                            newParcel.writeLong(id)
                            val customName = oldParcel.readString()
                            newParcel.writeString(customName)
                ...
                repeat(size) {
                    ...
                    when (oldParcel.readString()) {
                        "dev.vcgms.aapp.vaterialfiles.storage.SftpServer" -> {
                            ...
                            val authenticationPasswordOrPrivateKey = oldParcel.readString()
                            newParcel.writeString(authenticationPasswordOrPrivateKey)
                            if (authenticationClassName == "dev.vcgms.aapp.vaterialfiles.provider.sftp"
                ...
                Parcel.obtain().use { oldParcel ->
                    ...
                    readWriteLengthPrefixedValue(oldParcel, newParcel) {
                        ...
                        repeat(size) {
                            ...
                            oldParcel.setDataPosition(oldPosition)
                            when (className) {
                                "dev.vcgms.aapp.vaterialfiles.storage.DocumentManagerShortcut" -> {
                                    newParcel.writeInt(oldParcel.readInt())
                                    readWriteLengthPrefixedValue(oldParcel, newParcel) {
                                        oldParcel.readString()
                                        newParcel.writeString(
                                            "dev.vcgms.aapp.vaterialfiles.storage" +
